...
public class Tree<E> {
    private class Node<E> {
        ...
        public Node(E state, Node<E> parent, Action action, int cost) {
            this.state = state;
            this.parent = parent;
    ...
    private class Action {
        ...
        public List<DuckState> moveAction() {
            ...
            for (int i = 0; i < ducks.length; i++) {
                if (ducks[i].getPosition() < newState.getNumofPos() && ducks[i].getEnergy() > 0) { // If the duck's
                                                                                                   // position is not at
                                                                                                   // the end of the
                                                                                                   // board, and they
                                                                                                   // have enough
                    ...
                    if (ducks[i].hasCap() && ducks[i].getPosition() == newState.getNumofPos()) { // if has cap and at
                                                                                                 // the end of board
        ...
        private void movementDecison(Duck duck){
    ...
    private HashSet reach; // a set bc as the algorithms progresses through there's no telling of quantity
                           // of nodes
    ...
    private Node<E> breadthFirstSearchRecursive(E goal) {
        // O(1)
        // Starts at the root node, check is goal. If not, check to see if a left child
        // exists. If it does add it to the frontier, check to see if a right child
        // exist, if it does add it to the frontier.
